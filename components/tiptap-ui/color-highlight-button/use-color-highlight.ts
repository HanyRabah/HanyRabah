...
export function canColorHighlight(
  ...
  if (mode === "mark") {
    ...
    try {
      ...
      return editor.can().toggleNodeBackgroundColor?.("test") ?? false
    } catch {
      return false
    }
  }
...
export function removeHighlight(
  ...
  if (mode === "mark") {
    ...
    // @ts-ignore - unsetNodeBackgroundColor may not exist in all editor configurations
    return editor.chain().focus().unsetNodeBackgroundColor?.().run() ?? false
  }
}
